...
export function OnboardingTour({ isFirstTime, onComplete }: OnboardingTourProps) {
  ...
  const steps = useMemo<Step[]>(() => [
    {
      ...
      description: (
        <div className="flex flex-col gap-2">
          <p>Click  <button className="px-2 py-1 text-xs rounded bg-zinc-900 hover:bg-zinc-800 text-white w-fit transition-all">Use Template</button> to import one of our pre-made recipes into your collection.</p>
    ...
    },
    {
      ...
      description: (
        <div className="flex flex-col gap-2 items-center">
          ...
          <button className="w-12 h-12 mt-2 rounded-full flex items-center justify-center bg-zinc-900 hover:bg-zinc-800 shadow-lg transition-all"><span className="text-xl text-white">🎤</span></button>
        </div>
      ),
    ...
    },
    {
      ...
      description: (
        <div className="flex flex-col gap-2 items-center">
          ...
          <button className="w-48 mt-2 sm:w-auto px-4 py-2 rounded-lg flex items-center justify-center gap-2 bg-zinc-900 hover:bg-zinc-800 text-white transition-all">
  ...
  const highlightElement = (className: string) => {
    ...
    if (element) {
      ...
      element.classList.add('ring-4', 'ring-zinc-900', 'ring-opacity-50', 'transition-all', 'duration-500');
      setTimeout(() => {
        element.classList.remove('ring-4', 'ring-zinc-900', 'ring-opacity-50');
      }, 2000);
    }
  };
          ...
          <div className="relative bg-white dark:bg-gray-800 rounded-lg p-4 sm:p-6 w-[400px] min-h-[250px] max-w-[calc(100%-2rem)]">
            ...
            <div className="mt-2 flex justify-between items-center">
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  onClick={handleNext}
                  className="bg-zinc-900 hover:bg-zinc-800 text-white px-4 py-2 ml-2 rounded-full transition-all"
                >
